...
def boxify_local(odf):

    w = int(2.0 * SR / HOP)
    zmin = scipy.ndimage.minimum_filter1d(odf, w, mode='constant')
    odf = np.maximum(odf - zmin, 0.0)
    
    z = scipy.ndimage.maximum_filter1d(odf, w, mode='constant')
    z[z==0] = 1.0
...
def onset_linear_sum(S, *args, **kwargs):
    ...
    return boxify_local(np.sum(np.maximum(0.0, np.diff(S, axis=1)), axis=0))
...
def onset_linear_quantile(S, q=0.5):
    ...
    return boxify_local(np.array(scipy.stats.mstats.mquantiles(D, [q], axis=0)).flatten())
